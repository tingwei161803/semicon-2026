...
> 把 SEMICON Taiwan 2026「Transform Tomorrow」的展會內容，整理成一個多頁、可互動、雙語的靜態導覽站 —— 從展會總覽、20+ 國際論壇，到主題特區、展會週程、現場快報與互動小遊戲。

這是一個獨立、非官方的整理站，把 SEMICON Taiwan 2026 的總覽、論壇陣容、論壇時間軸、技術特區、活動週程、逛展參考與參觀須知，展會期間的現場快報，再加上七款半導體主題小遊戲，整理成 9 個彼此串連的頁面。內容整理自官方網站，純 HTML / CSS / JS、零 build，可直接部署到 GitHub Pages。
...
> 直接點進去就能用，無需安裝。頂部跨頁導覽列可在 9 頁之間切換；每頁皆支援中文 / English 與深 / 淺色，論壇頁可用 `…/forums.html#<slug>` 深連結到特定論壇。
...
| 現場快報 | `news.html` | **展前到展會期間的現場整理**（5 月啟動記者會起，以 8/31–9/4 展會週為主）：官方新聞稿、媒體報導、廠商發表與社群貼文共 86 則，依日期倒序排列，可搜尋與依來源類型篩選；**每一則都連到原始出處**。頁首有重點資訊圖輪播（中文 3 張、英文 4 張）與六條「今年舉辦的重點」歸納 |
...
| 互動小遊戲 | `game.html` | **七款半導體主題小遊戲**：晶片記憶配對、論壇快問快答（用站內資料自動出題）、晶圓分類反應、節點合成 2048、晶圓掃雷、製程排序、製程配方記憶；選單卡片進入、頁內切換，最佳分數記錄於 `localStorage` |
...
- 🎮 **互動小遊戲** — 七款半導體主題小遊戲（晶片記憶配對、論壇快問快答、晶圓分類反應、節點合成 2048、晶圓掃雷、製程排序、製程配方記憶），雙語與深淺色同步，最佳分數存於 `localStorage`
- 🔍 **即時搜尋 + 領域篩選** — 論壇頁可即時搜尋並依領域 chip 篩選
- 🪟 **詳情對話框 + 深連結** — 每場論壇可點開看完整介紹，網址帶 `#<slug>` 可分享
- 📰 **現場快報** — 展會期間的報導與貼文彙整，整張卡片就是通往原始出處的連結，依日期分組、可搜尋與依來源篩選
- 🖼️ **重點資訊圖輪播** — 中英各一套視覺整理（中文 3 張、英文 4 張），捲動吸附讓它沒有 JS 也能滑，圓點是真的錨點連結；箭頭、鍵盤左右鍵與自動輪播為漸進增強，讀者一動手就永久停止自動播放，並尊重 `prefers-reduced-motion`
- 🧱 **多種版型** — 複合首頁、卡片牆、bento 特區、時間軸、可展開 FAQ、來源連結時序牆，各取所長
...
「現場快報」則另外整理自展會期間（2026/8/31–9/4）的公開報導與公開貼文，來源包含 SEMI 官方新聞稿、中央社 Focus Taiwan、Taipei Times（含路透社供稿）、TrendForce、Digitimes Asia、電子時報、科技新報 TechNews、科技報橘、遠見、TVBS、NOWnews、三立、TaiwanPlus、中評社、mashdigi、廠商官方新聞稿與 SEMI 官方展商新聞稿平台，以及 LinkedIn / YouTube 上的公開貼文。

> 每一個連結在收錄前都實際開啟並與標題比對過；連不上或內容對不上的來源一律不列（例如一支查無此片的官方影片連結即被剔除）。數字均照原始報導引述，未經二次推算。

```
semicon-2026/
├── index.html / news.html / forums.html / schedule.html /   # 9 個共用 shell 的中文頁（根目錄）
│   zones.html / events.html / guide.html / visit.html / game.html
├── en/                 # 同名的 9 個英文頁（<html lang="en">，內容同樣來自 data.js）
├── assets/
│   ├── styles.css      # 設計 token（淺 / 深）+ 全部元件樣式（含小遊戲共用 UI kit）
│   ├── shell.js        # 共用 chrome：appbar / 跨頁導覽 / 頁尾 / dialog / 語言+主題狀態
│   ├── app.js          # 版型引擎：依 data-page 選 renderer 渲染進 #page（含 news 與 arcade 版型）
...
│   │   ├── flow.js         # 製程排序
│   │   └── recipe.js       # 製程配方記憶
│   ├── highlights/     # 現場快報頁的重點資訊圖（WebP，中文 3 張、英文 4 張）
│   └── favicon.svg
├── data/

...
(function () {
  ...
  function boot() {
    ...
    var RENDERERS = {
      ...
      /* ---- hub: composite home — hero, stats, themes, mission, explore ---- */
      hub: function (p) {
        var h = p.hero || {};

        var meta = (h.meta || []).map(function (m) {
          return '<span class="hero__meta-item">' +
            '<span class="material-symbols-rounded" aria-hidden="true">' + esc(m.icon || "circle") + "</span>" +
            esc(t(m.label)) + "</span>";
        }).join("");
        var ctas = (h.ctas || []).map(function (c) {
          return '<a class="btn ' + (c.subtle ? "btn--ghost" : "btn--primary") + '" href="' + esc(c.href) + '">' +
            "<span>" + esc(t(c.label)) + "</span>" +
            '<span class="btn__ic" aria-hidden="true"><span class="material-symbols-rounded">' + esc(c.icon || "arrow_outward") + "</span></span></a>";
        }).join("");
        var hero =
          '<section class="hero" data-reveal>' +
            (t(h.eyebrow) ? '<span class="hero__eyebrow">' + esc(t(h.eyebrow)) + "</span>" : "") +
            '<h1 class="hero__title">' + esc(t(h.headline) || t(p.title)) + "</h1>" +
            (t(h.lead) ? '<p class="hero__lead">' + esc(t(h.lead)) + "</p>" : "") +
            (meta ? '<div class="hero__meta">' + meta + "</div>" : "") +
            (ctas ? '<div class="hero__cta">' + ctas + "</div>" : "") +
          "</section>";

        var stats = (p.stats || []).map(function (s) {
          return '<div class="statband__item" data-item>' +
            '<b class="statband__num" data-count="' + esc(String(s.num)) + '" data-suffix="' + esc(s.suffix || "") + '">0</b>' +
            '<span class="statband__label">' + esc(t(s.label)) + "</span></div>";
        }).join("");
        var statband = stats ? '<section class="band" data-reveal><div class="statband">' + stats + "</div></section>" : "";

        function bandHead(title, sub) {
          return '<header class="band__head"><h2>' + esc(t(title)) + "</h2>" +
            (t(sub) ? "<p>" + esc(t(sub)) + "</p>" : "") + "</header>";
        }

        var themes = (p.themes || []).map(function (th) {
          return '<article class="themecard" data-item>' +
            '<span class="themecard__ic material-symbols-rounded" aria-hidden="true">' + esc(th.icon || "category") + "</span>" +
            '<h3 class="themecard__title">' + esc(t(th.title)) + "</h3>" +
            '<p class="themecard__body">' + esc(t(th.body)) + "</p></article>";
        }).join("");
        var themeBand = themes ?
          '<section class="band" data-reveal>' + bandHead(p.themesTitle, p.themesSub) +
            '<div class="themegrid">' + themes + "</div></section>" : "";

        var missionBand = "";
        if (p.mission && (p.mission.items || []).length) {
          var mi = p.mission.items.map(function (m) {
            return '<article class="misscard" data-item>' +
              '<span class="misscard__ic material-symbols-rounded" aria-hidden="true">' + esc(m.icon || "circle") + "</span>" +
              "<div><h3>" + esc(t(m.title)) + "</h3><p>" + esc(t(m.body)) + "</p></div></article>";
          }).join("");
          missionBand = '<section class="band band--mission" data-reveal>' + bandHead(p.mission.title) +
            '<div class="missiongrid">' + mi + "</div></section>";
        }
        ...
        var cards = L.pages.filter(function (q) { return q.slug !== "home"; }).map(function (q) {
          return '<a class="card card--nav" data-item href="' + esc(L.pageHref(q)) + '" ' +
            ...
            '<p class="card__summary">' + esc(t(q.subtitle)) + "</p>" +
            '<span class="card__go material-symbols-rounded" aria-hidden="true">arrow_outward</span></a>';
        }).join("");
        var exploreBand = '<section class="band" data-reveal>' + bandHead(p.exploreTitle || p.title) +
          '<div class="grid">' + cards + "</div></section>";

        var orgBand = "";
        if (p.organizers && (p.organizers.items || []).length) {
          var oi = p.organizers.items.map(function (o) {
            return '<div class="orgrow__item" data-item>' +
              '<span class="orgrow__role">' + esc(t(o.role)) + "</span>" +
              '<span class="orgrow__name">' + esc(t(o.name)) + "</span></div>";
          }).join("");
          orgBand = '<section class="band" data-reveal><div class="orgrow">' +
            '<span class="orgrow__title">' + esc(t(p.organizers.title)) + "</span>" +
            '<div class="orgrow__items">' + oi + "</div></div></section>";
        }

        return hero + statband + themeBand + missionBand + exploreBand + orgBand;
      },
      ...
      faq: function (p) {
        var items = (p.qa || []).map(function (row) {
          var links = (row.links || []).map(function (lk) {
            var ext = /^https?:/i.test(lk.href || "");
            return '<a class="acc-link" href="' + esc(lk.href) + '"' +
              (ext ? ' target="_blank" rel="noopener"' : "") + '>' +
              '<span>' + esc(t(lk.label)) + "</span>" +
              '<span class="material-symbols-rounded" aria-hidden="true">' + (ext ? "open_in_new" : "arrow_forward") + "</span></a>";
          }).join("");
          return '<details class="acc-item" data-item data-q="' + esc((t(row.q) + " " + t(row.a)).toLowerCase()) + '">' +
            ...
            '<div class="acc-a">' + esc(t(row.a)) +
            (links ? '<div class="acc-links">' + links + "</div>" : "") + "</div></details>";
        }).join("");
        return head(p) +
    ...
    /* ---- prefers-reduced-motion (sandbox-safe) ---- */
    function reducedMotion() {
      try { return window.matchMedia("(prefers-reduced-motion: reduce)").matches; }
      catch (e) { return false; }
    }

    /* ---- hero count-up (shared by hub): numeric core + suffix, comma-formatted ---- */
    function animateCounters() {
      ...
      function run(el) {
        ...
        function step(ts) {
          ...
          if (pr < 1) requestAnimationFrame(step); else el.textContent = fmt(target) + suffix;
        }
    ...
    /* ---- scroll reveal: fade-up [data-reveal] sections as they enter view ---- */
    function revealize() {
      var items = [].slice.call(pageEl.querySelectorAll("[data-reveal]"));
